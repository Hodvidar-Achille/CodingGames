...
class SpeedCalculator
{
	...
	public void addRoadSegment(RoadSegment aRoadSegment)
	{
		...
		this.min = min;
		this.max = max;
		Solution.printIfVerbose("addRoadSegment new min("+min+") and max("+max+") values.");
	}
	private List<Interval> getIntervalsForRoadSegment(RoadSegment aRoadSegment)
	{
		...
		while (true)
		{
			minSpeedToPass = getSpeedInKmPerHour(distance, (duration - Solution.MINIMUM_TIME), Solution.FLOOR_MIN_SPEED);
			if(minDuration == 0)
				maxSpeedToPass = this.max;
			else
				maxSpeedToPass = getSpeedInKmPerHour(distance, minDuration, Solution.FLOOR_MAX_SPEED);
			...
			if (cappedMinSpeedToPass > this.max)
			{
				...
				Solution.printIfVerbose("Discard illegal speed km/h : " + speedInterval.toString()
					...
					+ (duration - Solution.MINIMUM_TIME - durationIncrement)
					+ " seconds.");
				continue;
			}
	 ...
	 * @return
	 */
	public static int getSpeedInKmPerHour(int distanceInMeter, int timeInSecond, boolean useFloor)
	{
		if(useFloor)
