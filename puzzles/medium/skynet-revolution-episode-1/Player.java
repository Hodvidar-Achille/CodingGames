...
class Player
{
	...
	public static void main(String args[])
	{
		...
		while (true)
		{
			...
			else
			{
				...
				final int[] pred = DijkstraNode.dijkstra(nodes, SI);
				List<ArrayList<Node>> paths = new ArrayList<>();
				for (int e : exists)
				{
					path = DijkstraNode.getPath(nodes, pred, SI, e);
					paths.add(path);
					if (path.size() == 2)
						break;
				}
	...
	private static Node[] removeAndCut(
	...
	private static Node[] removeAndCut(List<Edge> existEdges, Node node1, Node node2, Node[] nodes)
	{
		node1.cutCouble(node2);
	...
	private static Node[] cutFromGraphOrphanNodes(Node[] nodes)
	{
...
class DijkstraNode
{
	public static int[] dijkstra(Node[] nodes, int s)
	{
	...
	public static ArrayList<Node> getPath(Node[] nodes, int[] pred, int s, int dest)
	{
