...
class CylindersTest {
    ...
    void boxWidthForSeveralCylindersOptimized(final String cylindersArray,
                                              final double expected) {
        Assertions.assertThat(DoubleFormater.roundTo3rdDecimal(
                        Cylinders.getMinLength(NumberExtractor.getArray(cylindersArray))))
                .isEqualTo(expected);
    }
    ...
    void boxWidthForSeveralCylindersOptimizedUsingShuffle(final String cylindersArray,
                                                          final double expected) {
        Assertions.assertThat(DoubleFormater.roundTo3rdDecimal(
                        Cylinders.getMinLengthUsingSuffle(NumberExtractor.getArray(cylindersArray),
                                1000000)))
                .isEqualTo(expected);
    }
    ...
    void boxWidthForSeveralCylindersOptimizedAsString(final String cylindersArray,
                                                      final String expected) {
        final DecimalFormat formatter = new DecimalFormat("0.000", DecimalFormatSymbols.getInstance(Locale.ENGLISH));
        formatter.setRoundingMode(RoundingMode.HALF_EVEN);
        Assertions.assertThat(formatter.format(Cylinders.getMinLength(NumberExtractor.getArray(cylindersArray))))
                .isEqualTo(expected);
    }
}
