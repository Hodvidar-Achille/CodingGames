...
public final class Day7_2
{
	...
	private static int test(String inputFile) throws Exception
	{
		...
		int maxOutput = 0;
		int phaseCounter = 0;
		Collection<String> phases = new CodeGenerator(5, 9).generateUniqueDigitCode(5);
	...
	private static int tryPhase(String phase, double[] opCode)
	{
		...
		Amplifier A = new Amplifier(opCode.clone(), p);
		p = getPhaseValue(phase, 1);
		Amplifier B = new Amplifier(opCode.clone(), p);
		p = getPhaseValue(phase, 2);
		Amplifier C = new Amplifier(opCode.clone(), p);
		p = getPhaseValue(phase, 3);
		Amplifier D = new Amplifier(opCode.clone(), p);
		p = getPhaseValue(phase, 4);
		Amplifier E = new Amplifier(opCode.clone(), p);
