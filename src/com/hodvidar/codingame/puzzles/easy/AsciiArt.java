...
/**
 * https://www.codingame.com/ide/puzzle/ascii-art by Hodvidar
 **/
public class AsciiArt {
	...
	public static void main(final String[] args) {
		...
		final int longueurResultat = T.length();
		final char[] motEcrit = T.toCharArray();
		// Largeur * (26 lettres + 1 signe) + 26 espace entre les 26 premières lettres
		final int LTotale = L * 27 + 26;
		final char[][] tab = new char[H][LTotale];
		final char[][] motResultat = new char[H][longueurResultat * L];
		...
		// test contenu
		/*
		 * for(int i = 0; i < alphabet.length; i++){ System.err.print("_"+alphabet[i]); }
		 */
		System.err.println();
		int i2 = 0, j2 = 0;
		boolean boo = false;
				...
				if (motEcrit[i] == alphabet[j]) {
					...
					for (int a = 0; a < H; a++) {
						for (int b = 0; b < L; b++) {
							...
							// System.err.println("ligne test E,["+a+"]["+(j+b)+"] signe recupere :
							// "+tab[a][j+b]);
						}
					}
					boo = true;
		...
		for (int i = 0; i < H; i++) {
			final String ligneResult = new String(motResultat[i]);
