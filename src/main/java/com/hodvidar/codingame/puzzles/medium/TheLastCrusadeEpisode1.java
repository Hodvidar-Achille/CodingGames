...
class TheLastCrusadeEpisode1 {
    ...
    public static void main(final String[] args) {
        ...
        for (int i = 0; i < H; i++) {
            ...
            final String[] lineChar = LINE.split("\\s+");
            for (final String s : lineChar) {
                final int n = Integer.parseInt(s);
                grid[j][i] = n;
                j++;
            }
        }
        final int EX = in.nextInt(); // the coordinate along the X axis of the exit (not useful for this first mission, but must be read).
        ...
        while (true) {
            ...
            // GO ! 
            final int typeRoom = grid[XI][YI];
            final String nextRoom = computeMove(XI, YI, POS, typeRoom);
    ...
    private static String computeMove(int x, int y, final String entryMove, final int t) {
