...
/**
 * https://www.isograd.com/FR/solutionconcours.php?contest_id=50 Solution is faster than Solution
 * (PASSED even if theses test failed). by Hodvidar
 */
public final class Crapette {
    ...
    private static String test(final String inputFile) throws FileNotFoundException {
        ...
        TopOfPile[] piles = {};
        int numberOfCard = 0;
        final TopOfPile startPile = new TopOfPile();
        while (sc.hasNextLine()) {
            line = sc.nextLine();
            /* Lisez les données et effectuez votre traitement */
            printIfVerbose("i=" + i + " - line=" + line);
            if (i == 0) {
                final int emptyPiles = Integer.parseInt(line);
                piles = new TopOfPile[emptyPiles];
            } else if (i == 1) {
                numberOfCard = Integer.parseInt(line);
            } else {
                final String[] cardsStr = line.split("\\s+");
                for (final String s : cardsStr) {
                    final int card = Integer.parseInt(s);
    ...
    /**
     * For N Cards with N Piles Move c1 to p1, c2 to p2..., cn to pn. Then move cn-1 from pn-1 to
     * pn, cn-2 to pn..., c1 to pn;
     *
     * @param mover
     */
    private static void moveNCardsWithNPiles(final MouvementWriter mover, final int numberOfCards) {
        final int max = numberOfCards - 1;
    ...
    private static void moveNCardsWithMinusOnePiles(final MouvementWriter mover) {
        // take out cards
    ...
    private static void moveNCardsWithMinusTwoPiles(final MouvementWriter mover) {
        // take out cards
        for (int i = 0; i < mover.piles.length; i++) {
            mover.moveFromStart(i);
        }
     ...
     * @param mover
     */
    private static void moveNCardsWithMinusThreePiles(final MouvementWriter mover) {
        // take out cards
        for (int i = 0; i < mover.piles.length; i++) {
            mover.moveFromStart(i);
        }
    ...
    private static final class MouvementWriter {
        ...
        public MouvementWriter(final TopOfPile[] piles, final TopOfPile startPile) {
            this.piles = piles;
        ...
        public void move(final int indexFrom, final int indexTo) {
            final int cardToMove = piles[indexFrom].takeCard();
        ...
        public void moveFromStart(final int indexTo) {
            final int cardToMove = startPile.takeCard();
            piles[indexTo].putCard(cardToMove);
            if (!"".equals(movements))
                movements += ";";
            movements += cardToMove + " " + (indexTo + 1);
    ...
    public static final class TopOfPile {
        ...
        public void putCard(final int aCard) {
        ...
        public int takeCard() {
            ...
            final int cardToReturn = this.card;
            if (this.next == null) {
                this.card = 0;
            } else {
 ...
 *
 * Dans ce jeu, 8 cases sont utilisées pour y placer temporairement des piles de cartes en séries
 * décroissantes de couleur alternante. Pour réussir, les bons joueurs réunissent ces piles dans le
 * but de maximiser le nombre de cases vides.
 *
 * Votre but est d'écrire un programme permettant de déplacer une pile de cartes consécutives
 * ordonnées de façon décroissante depuis une case de départ vers autre une case en respectant les
 * règles suivantes :- On ne peut déplacer qu'une carte à la fois. - Seule la carte du dessus d'une
 * pile peut être déplacée. - Une carte 2 doit être posée sur un 3, un 3 sur un 4, une dame sur un
 * roi, etc. Il n'y a pas d'as. - N'importe quelle carte peut-être posée sur une case vide. Dans
 * cette version simplifiée du jeu, on ne tient pas compte des couleurs des cartes.
 *
 * Données
 *
 * Entrée
 *
 * Ligne 1 : un entier compris entre 1 et 7 (inclus) représentant le nombre de piles vides Ligne 2 :
 * un entier N compris entre 1 et 7 représentant le nombre de cartes sur la pile de départ Ligne 3 :
 * N entiers compris entre 2 et 13 séparés par des espaces représentant les cartes de la pile de
 * départ (11, 12 et 13 représentent valet, dame et roi), du bas vers le haut. Les cartes sont
 * ordonnées de façon décroissante et consécutives. La carte la plus petite est sur le dessus.
 *
 * Sortie
 *
 ...
 * déplacer, puis un espace, puis un entier décrivant le numéro de la pile où placer la carte (la
 * pile de départ est numérotée 0, les piles vides de 1 jusqu'à N).
 *
 * À l'issue de ces commandes, la pile de départ doit avoir été reconstituée à l'identique sur
 * n'importe laquelle des autres piles. Toutes les piles restantes doivent êtres vides.
 *
 * Exemple
 *
 * Entrée
